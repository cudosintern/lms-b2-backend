...
@router.get("/programs")
def get_programs(dept_id: int, db: Session = Depends(get_db), current_user: dict = Depends(get_current_user)):
    try:
        sql = "SELECT pgm_id, pgm_title, pgm_acronym FROM iems_program WHERE dept_id = :dept_id AND status = 1"
